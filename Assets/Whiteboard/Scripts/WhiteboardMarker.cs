    ...
    private void Draw()
    {
        if (Physics.Raycast(tip.position, transform.up, out touch, tipHeight))
        {
            if (touch.transform.CompareTag("Whiteboard"))
            {
                ...
                if (y < 0 || y > whiteboard.textureSize.y || x < 0 || x > whiteboard.textureSize.y) return;


                if (touchedLastFrame)
                {
                    whiteboard.texture.SetPixels(x, y, penSize, penSize, colors);
                    for (float f = 0.01f; f < 1.00f; f += 0.01f) //letzen Wert ändern, um weniger Coverage zu haben
                    {
